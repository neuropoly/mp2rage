...
class Option:
    ...
    def check_integrity(self, param, type=None):
        ...
        if type_option in self.OPTION_TYPES:
            ...
            if not check_write_permission(param):
            ...
            printv("WARNING : Option "+str(self.type_value)+" does not exist and will have no effect on the execution of the script", "warining")
            printv("Type -h to see supported options", "warning")
    ...
    def checkFile(self, param):
        ...
        printv("Check file existence...", 0)
        if self.parser.check_file_exist:
            check_file_exist(param, 0)
        return param
    ...
    def checkIfNifti(self, param):
        ...
        if param.lower().endswith('.nii'):
            ...
            printv("ERROR: File is not a NIFTI image file. Exiting", type='error')
        ...
        if nii:
            ...
            printv("ERROR: File "+param+" does not exist. Exiting", type='error')

    def checkFolder(self, param):
        # check if the folder exist. If not, create it.
        if self.parser.check_file_exist:
            printv("Check folder existence...")
            check_folder_exist(param, 0)
        return param
    ...
    def checkFolderCreation(self, param):
        ...
        printv("Check folder existence...")
        if self.parser.check_file_exist:
            result_creation = create_folder(param)
        else:
            result_creation = 0  # no need for checking
        if result_creation == 2:
            printv("ERROR: Permission denied for folder creation...", type="error")
        elif result_creation == 1:
            printv("Folder "+param+" has been created.", 0, type='warning')
        # add slash at the end
        param = slash_at_the_end(param, 1)
        return param
        ...
        for index, arg in enumerate(arguments):
            ...
            if arg in self.options:
                if self.options[arg].deprecated_rm:
                    printv("ERROR : "+arg+" is a deprecated argument and is no longer supported by the current version.", 1, 'error')
                # for each argument, check if is in the option list.
                # if so, check the integrity of the argument
                if self.options[arg].deprecated:
                    printv("WARNING : "+arg+" is a deprecated argument and will no longer be updated in future versions.", 1, 'warning')
                if self.options[arg].deprecated_by is not None:
                    try:
                        printv("WARNING : "+arg+" is a deprecated argument and will no longer be updated in future versions. Changing argument to "+self.options[arg].deprecated_by+".", 1, 'warning')
                        arg = self.options[arg].deprecated_by
                    except KeyError as e:
                        printv("ERROR : Current argument non existent : " + e.message, 1, 'error')
    ...
    def add_path_to_file(self, dictionary, path_to_add, input_file=True, output_file=False, do_not_add_path=[]):
        ...
        for key, option in dictionary.iteritems():
            ...
            if key in self.options:
                ...
                printv("ERROR: the option you provided is not contained in this parser. Please check the dictionary", verbose=1, type='error')
...
# ======================================================================================================================
# USEFUL FUNCTIONS FROM sct_utils
# ======================================================================================================================
...
def check_write_permission(fname):
    """
